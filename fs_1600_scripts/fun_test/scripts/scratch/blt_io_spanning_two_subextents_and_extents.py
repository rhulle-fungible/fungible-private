...
class BLTIOSpanningTwoSubextentsScript(FunTestScript):
    def describe(self):
...
class BLTIOSpanningTwoSubextentTestcase(FunTestCase):
...
class BLTIOTwoSubextents(BLTIOSpanningTwoSubextentTestcase):

    def describe(self):
        self.set_test_details(id=1,
                              summary="IO spanning two sub-extents",
                              steps="""
        1. Create BLT volume of 1GB using nvme create-ns
        2. Attach volume using nvme attach-ns
        3. Using nvme write, Write 8KB of data such as it will span on two sub-extents
        4. Verify two sub-extents are created
        5. Using nvme read, Read 8KB data starting from same block
        6. Verify md5sum of input and output is same
        """)

    def setup(self):
        super(BLTIOTwoSubextents, self).setup()
    ...
    def cleanup(self):
        super(BLTIOTwoSubextents, self).cleanup()


if __name__ == "__main__":
    blt_io_spanning_two_subextents = BLTIOSpanningTwoSubextentsScript()
    blt_io_spanning_two_subextents.add_test_case(BLTIOTwoSubextents())
    blt_io_spanning_two_subextents.run()
