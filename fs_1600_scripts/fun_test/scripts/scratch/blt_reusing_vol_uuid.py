...
class FunTestCase1(FunTestCase):
    def describe(self):
        self.set_test_details(id=1,
                              summary="Reusing uuid and volume-name of deleted volume",
                              steps="""
        ...
        9. Create volume with same uuid and volume name
        10. Perform IO and validate data
                              """)
    ...
    def run(self):
        ...
        for iterator in range(0, 2, 1):
            ...
            fun_test.test_assert(result_attach_volume["status"],
                                 "Iter{}: Thin Block volume is attached".format(iterator))
            ...
            fun_test.test_assert_expected(expected=initial_counter_stat, actual=volume_status_write,
                                          message="Iter{}: Write counter is correct".format(iterator))
            fun_test.test_assert_expected(expected=initial_counter_stat, actual=volume_status_read,
                                          message="Iter{}: Read counter is correct".format(iterator))
            ...
            fun_test.simple_assert(command_result["status"],
                                   "Iter{}: Volume stats of DUT Instance 0 after IO".format(iterator))
            ...
            fun_test.test_assert(result_detach_volume["status"],
                                 "Iter{}: Thin Block volume is detached".format(iterator))

            command_result = storage_controller.peek(storage_props_tree)
            fun_test.log(command_result)
            fun_test.test_assert_expected(expected=True, actual=command_result["status"],
                                          message="Iter{}: Volume is detached".format(iterator))
            ...
            '''
            # Ensure volume is really deleted. This should return error
            command_result = storage_controller.peek(storage_props_tree)
            fun_test.log(command_result)
            fun_test.test_assert_expected(expected=False, actual=command_result["status"],
                                          message="Iter{}: Stat and Counters are reset after "
                                                  "deleting volume".format(iterator))
            '''
